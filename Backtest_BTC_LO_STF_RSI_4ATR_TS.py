...
class CustomPandas(bt.feeds.PandasData):
    # Add a 'stf2sd' line to the inherited ones from the base class
    #lines = ('stf2sd',)
    lines = ('stf',)
...
# Get On-chain data from Amberdata - Stock to flow indicator
...
class Strategy(bt.Strategy):
    params = (
        ...
        ('atrmultiple', 4.0), # ATR Multiple for stop
    )
    ...
    def next(self):
        ...
        if not self.position: # Verify we are flat
            if self.data.close < self.data.stf and self.rsi < 30:
                self.order = self.buy()
                print('buy at:', self.data.close[0],'STF:', self.data.stf[0], 'RSI:', self.rsi[0])
                pdist = self.atr[0] * self.p.atrmultiple
                self.pstop = self.data.close[0] - pdist

        else:  # Position in Market
            pclose = self.data.close[0]
            pstop = self.pstop

            if pclose < pstop:
                self.close()  # Stop criteria met
                print('closed', self.data.close[0])
            else:
                pdist = self.atr[0] * self.p.atrmultiple # Trail my stop up
